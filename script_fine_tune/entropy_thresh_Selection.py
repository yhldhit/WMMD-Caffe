...
entropy_thresh_list = [0.4,0.7,1.0,1.4,1.7,2.0,10]
#kernel_num_list = [1,3,5,7,10]
for entropy_thresh in entropy_thresh_list:
    print entropy_thresh
   ...
   #fine tune net in this part
    if Net_TUNE:
        #fine tune thresh
        entropy_thresh_tmp = {
            'mmd_fc7':entropy_thresh,
            'mmd_fc8':entropy_thresh,
        }

        for layer in net.layer:
            name  = layer.name
            if name in entropy_thresh_tmp:
                layer.mmd_param.entropy_thresh = entropy_thresh
